...
object S3cp {
  def process(inputDir: Path,
    ...
    for {
      ...
      modelLists = groupByModel(files.toList)
      _     <- if (skipSchemaLists) EitherT.rightT[IO, NonEmptyList[Common.Error]](Nil)
                 else uploadModelLists(modelLists, pathPrefix, bucketName, s3)
      _     <- if (skipSchemaLists) EitherT.rightT[IO, NonEmptyList[Common.Error]](Nil)
                 else uploadTopLevelList(modelLists, pathPrefix, bucketName, s3)
    } yield Nil
  ...
  def groupByModel(files: List[SchemaFile]): Map[Path, List[String]] =
    files
      .map(_.content.self.schemaKey)
      .groupBy(k => (k.vendor, k.name, k.version.model))
      .map { case ((vendor, name, model), schemaKeys) =>
