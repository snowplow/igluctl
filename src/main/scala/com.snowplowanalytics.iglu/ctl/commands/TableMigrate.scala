...
object TableMigrate {
  ...
  private[ctl] def getResult(tableCheckResult: TableCheckResult, outputS3Path: String, awsRole: String, awsRegion: String): Either[Common.Error, String] = {
    tableCheckResult match {
      case TableNotDeployed(_) => Common.Error.Message("Table not deployed").asLeft
      case TableMatched(_) => Common.Error.Message("Table matched").asLeft
      case TableUnmatched(_, existingColumns, expectedColumns) if !checkIfColumnsSame(existingColumns, expectedColumns) => Common.Error.Message("Columns are different, can not create migration statements").asLeft
      case TableUnmatched(schemaKey, _, expectedColumns) => createMigrationStatements(expectedColumns, outputS3Path, schemaKey, awsRole, awsRegion).asRight
    }
  }
  ...
  private def checkIfColumnsSame(existingColumns: List[Column], expectedColumns: List[DDLColumn]): Boolean =
