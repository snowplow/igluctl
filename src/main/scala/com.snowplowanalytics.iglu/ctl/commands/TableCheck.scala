...
object TableCheck {
    ...
    val stream = for {
      ...
      res     <- {
        val checkRes: Failing[List[TableCheckResult]] = (resolver, schemaKey, igluServerUrl, apikey) match {
          ...
          case (None, None, Some(igluUrl), Some(a)) =>
  ...
  /**
    ...
    */
  def tableCheckMultiple(registryRoot: Server.HttpUrl, masterApiKey: UUID, storage: Storage[IO], dbschema: String): Stream[Failing, TableCheckResult] =
    for {
      keys        <- Stream.resource(Server.temporaryKeys(registryRoot, masterApiKey))
      schemaJsons <- Stream.eval(Pull.getSchemas(Server.buildPullRequest(registryRoot, keys.read)))
