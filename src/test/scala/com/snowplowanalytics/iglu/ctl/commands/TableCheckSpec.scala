...
class TableCheckSpec extends Specification { def is =s2"""
  ...
  def e1 = {
    assertUnmatched(
      existingComment = Some(Comment("iglu:com.vendor/example/jsonchema/1-0-0")),
  ...
  def e6 = {
    val existingComment = Some(Comment("iglu:com.vendor/example/jsonchema/1-0-0"))
  ...
  def e8 = {
    assertUnmatched(
      existingComment = Some(Comment("iglu:com.vendor_another/example/jsonchema/1-0-0")),
      existingColumns = List(int("a"), int("b"), int("c"), int("d"), int("e")),
      expectedColumns = List(int("a"), int("b"), int("c"), int("d"), int("e")),
      result = List(CommentProblem("SchemaKey found in table comment [iglu:com.vendor_another/example/jsonchema/1-0-0] does not match expected [iglu:com.vendor/example/jsonschema/1-0-0]"))
    )
  }
